...
namespace LuaGlobalFunctions
{
     ...
     */
    int GetRealmID(Forge* F)
    {
        F->Push(sConfigMgr->GetIntDefault("RealmID", 1));
        return 1;
    }
     ...
     * @return [Map] map
     */
    int GetStateMap(Forge* F)
    {
        F->Push(F->GetBoundMap());
        return 1;
    }
     ...
     * @return int32 mapId
     */
    int GetStateMapId(Forge* F)
    {
        F->Push(F->GetBoundMapId());
        return 1;
    }
     ...
     */
    int GetStateInstanceId(Forge* F)
    {
        F->Push(F->GetBoundInstanceId());
        return 1;
    }
     ...
     * @return [Quest] quest
     */
    int GetQuest(Forge* F)
    {
        uint32 questId = F->CHECKVAL<uint32>(1);

        F->Push(eObjectMgr->GetQuestTemplate(questId));
        return 1;
    }
     ...
     */
    int GetPlayerByGUID(Forge* F)
    {
        ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);
        F->Push(eObjectAccessor()FindPlayer(guid));
        return 1;
    }
     ...
     * @return [Player] player
     */
    int GetPlayerByName(Forge* F)
    {
        const char* name = F->CHECKVAL<const char*>(1);
        F->Push(eObjectAccessor()FindPlayerByName(name));
        return 1;
    }
     ...
     * @return uint32 time
     */
    int GetGameTime(Forge* F)
    {
        F->Push(uint32(GameTime::GetGameTime()));
        return 1;
    }
     ...
     */
    int GetPlayersInWorld(Forge* F)
    {
        uint32 team = F->CHECKVAL<uint32>(1, TEAM_NEUTRAL);
        bool onlyGM = F->CHECKVAL<bool>(2, false);
    ...
    int GetPlayersOnMap(Forge* F)
    {
        ...
        Map::PlayerList const& players = F->GetBoundMap()->GetPlayers();
        if (!players.isEmpty())
        {
     ...
     */
    int GetGuildByName(Forge* F)
    {
        const char* name = F->CHECKVAL<const char*>(1);
        F->Push(eGuildMgr->GetGuildByName(name));
        return 1;
    }
     ...
     */
    int GetMapById(Forge* F)
    {
        uint32 mapid = F->CHECKVAL<uint32>(1);
        uint32 instance = F->CHECKVAL<uint32>(2, 0);

        F->Push(eMapMgr->FindMap(mapid, instance));
        return 1;
    }
     ...
     */
    int GetGuildByLeaderGUID(Forge* F)
    {
        ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);

        F->Push(eGuildMgr->GetGuildByLeader(guid));
        return 1;
    }
     ...
     * @return uint32 count
     */
    int GetPlayerCount(Forge* F)
    {
        F->Push(eWorld->GetActiveSessionCount());
        return 1;
    }
     ...
     */
    int GetPlayerGUID(Forge* F)
    {
        uint32 lowguid = F->CHECKVAL<uint32>(1);
        F->Push(MAKE_NEW_GUID(lowguid, 0, HIGHGUID_PLAYER));
        return 1;
    }
     ...
     * @return ObjectGuid guid
     */
    int GetItemGUID(Forge* F)
    {
        uint32 lowguid = F->CHECKVAL<uint32>(1);
        F->Push(MAKE_NEW_GUID(lowguid, 0, HIGHGUID_ITEM));
        return 1;
    }
     ...
     * @return ObjectGuid guid
     */
    int GetObjectGUID(Forge* F)
    {
        uint32 lowguid = F->CHECKVAL<uint32>(1);
        uint32 entry = F->CHECKVAL<uint32>(2);
        F->Push(MAKE_NEW_GUID(lowguid, entry, HIGHGUID_GAMEOBJECT));
        return 1;
    }
     ...
     * @return ObjectGuid guid
     */
    int GetUnitGUID(Forge* F)
    {
        uint32 lowguid = F->CHECKVAL<uint32>(1);
        uint32 entry = F->CHECKVAL<uint32>(2);
        F->Push(MAKE_NEW_GUID(lowguid, entry, HIGHGUID_UNIT));
        return 1;
    }
     ...
     */
    int GetGUIDLow(Forge* F)
    {
        ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);

        F->Push(guid.GetCounter());
        return 1;
    }
     ...
     */
    int GetItemLink(Forge* F)
    {
        uint32 entry = F->CHECKVAL<uint32>(1);
        ...
        if (!temp)
            return luaL_argerror(F->L, 1, "valid ItemEntry expected");
     ...
     */
    int GetGUIDType(Forge* F)
    {
        ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);
        F->Push(static_cast<int>(guid.GetHigh()));
        return 1;
    }
     ...
     */
    int GetGUIDEntry(Forge* F)
    {
        ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);
        F->Push(guid.GetEntry());
        return 1;
    }
     ...
     */
    int GetAreaName(Forge* F)
    {
        uint32 areaOrZoneId = F->CHECKVAL<uint32>(1);
        uint8 locale = F->CHECKVAL<uint8>(2, DEFAULT_LOCALE);
        if (locale >= TOTAL_LOCALES)
            return luaL_argerror(F->L, 2, "valid LocaleConstant expected");
        ...
        if (!areaEntry)
            return luaL_argerror(F->L, 1, "valid Area or Zone ID expected");

        F->Push(areaEntry->AreaName[locale]);
        return 1;
    }
     ...
     */
    int GetActiveGameEvents(Forge* F)
    {
        lua_newtable(F->L);
        int tbl = lua_gettop(F->L);
        uint32 counter = 1;
    ...
    static int RegisterEntryHelper(Forge* F, int regtype)
    {
        uint32 id = F->CHECKVAL<uint32>(1);
        uint32 ev = F->CHECKVAL<uint32>(2);
        luaL_checktype(F->L, 3, LUA_TFUNCTION);
        uint32 shots = F->CHECKVAL<uint32>(4, 0);

        lua_pushvalue(F->L, 3);
        int functionRef = luaL_ref(F->L, LUA_REGISTRYINDEX);
        if (functionRef >= 0)
            return F->Register(regtype, id, ObjectGuid(), 0, ev, functionRef, shots);
        else
            luaL_argerror(F->L, 3, "unable to make a ref to function");
        return 0;
    }

    static int RegisterEventHelper(Forge* F, int regtype)
    {
        uint32 ev = F->CHECKVAL<uint32>(1);
        luaL_checktype(F->L, 2, LUA_TFUNCTION);
        uint32 shots = F->CHECKVAL<uint32>(3, 0);

        lua_pushvalue(F->L, 2);
        int functionRef = luaL_ref(F->L, LUA_REGISTRYINDEX);
        if (functionRef >= 0)
            return F->Register(regtype, 0, ObjectGuid(), 0, ev, functionRef, shots);
        else
            luaL_argerror(F->L, 2, "unable to make a ref to function");
        return 0;
    }

    static int RegisterUniqueHelper(Forge* F, int regtype)
    {
        ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);
        uint32 instanceId = F->CHECKVAL<uint32>(2);
        uint32 ev = F->CHECKVAL<uint32>(3);
        luaL_checktype(F->L, 4, LUA_TFUNCTION);
        uint32 shots = F->CHECKVAL<uint32>(5, 0);

        lua_pushvalue(F->L, 4);
        int functionRef = luaL_ref(F->L, LUA_REGISTRYINDEX);
        if (functionRef >= 0)
            return F->Register(regtype, 0, guid, instanceId, ev, functionRef, shots);
        else
            luaL_argerror(F->L, 4, "unable to make a ref to function");
        return 0;
    }
     ...
     * @values [16, FORGE_EVENT_ON_LUA_STATE_CLOSE, "ALL", <event: number>, "Triggers just before shutting down the Forge state (on shutdown, restart and reload)"]
     ...
     * @values [33, FORGE_EVENT_ON_LUA_STATE_OPEN, "ALL", <event: number>, "Triggers after all scripts are loaded"]
     ...
     */
    int RegisterServerEvent(Forge* F)
    {
        return RegisterEventHelper(F, Hooks::REGTYPE_SERVER);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterPlayerEvent(Forge* F)
    {
        return RegisterEventHelper(F, Hooks::REGTYPE_PLAYER);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterGuildEvent(Forge* F)
    {
        return RegisterEventHelper(F, Hooks::REGTYPE_GUILD);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterGroupEvent(Forge* F)
    {
        return RegisterEventHelper(F, Hooks::REGTYPE_GROUP);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterBGEvent(Forge* F)
    {
        return RegisterEventHelper(F, Hooks::REGTYPE_BG);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterPacketEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_PACKET);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterCreatureGossipEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_CREATURE_GOSSIP);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterGameObjectGossipEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_GAMEOBJECT_GOSSIP);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterItemEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_ITEM);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterItemGossipEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_ITEM_GOSSIP);
    }
     ...
     * @param uint32 shots = 0 : the number of times the function will be called, 0 means "always call this function"
     */
    int RegisterMapEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_MAP);
    }
     ...
     * @param uint32 shots = 0 : the number of times the function will be called, 0 means "always call this function"
     */
    int RegisterInstanceEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_INSTANCE);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterPlayerGossipEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_PLAYER_GOSSIP);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterCreatureEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_CREATURE);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterUniqueCreatureEvent(Forge* F)
    {
        return RegisterUniqueHelper(F, Hooks::REGTYPE_CREATURE);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterGameObjectEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_GAMEOBJECT);
    }
     ...
     * @return function cancel : a function that cancels the binding when called
     */
    int RegisterSpellEvent(Forge* F)
    {
        return RegisterEntryHelper(F, Hooks::REGTYPE_SPELL);
    }

    /**
     * Reloads the Lua engine.
     */
    int ReloadForge(Forge* F)
    {
        F->ReloadForge();
        return 0;
    }
     ...
     */
    int RunCommand(Forge* F)
    {
        const char* command = F->CHECKVAL<const char*>(1);
     ...
     */
    int SendWorldMessage(Forge* F)
    {
        const char* message = F->CHECKVAL<const char*>(1);
     ...
     */
    int WorldDBQuery(Forge* F)
    {
        const char* query = F->CHECKVAL<const char*>(1);
     ...
     * The callback function parameter is the query result (an [ForgeQuery] or nil if no rows found).
     *
    ...
    int WorldDBQueryAsync(Forge* F)
    {
        ...
        lua_pushvalue(F->L, 2);
        int funcRef = luaL_ref(F->L, LUA_REGISTRYINDEX);
        ...
        F->GetQueryProcessor().AddCallback(WorldDatabase.AsyncQuery(query).WithCallback([F, funcRef](QueryResult result)
        {
            ForgeQuery* eq = result ? &result : nullptr;

            // Get the Lua function from the registry
            lua_rawgeti(F->L, LUA_REGISTRYINDEX, funcRef);
            ...
            luaL_unref(F->L, LUA_REGISTRYINDEX, funcRef);
        }));
        return 0;
    }
     ...
     */
    int CharDBQuery(Forge* F)
    {
        const char* query = F->CHECKVAL<const char*>(1);

        ForgeQuery result = CharacterDatabase.Query(query);
        if (result)
            F->Push(&result);
        else
            F->Push();
    ...
    int CharDBQueryAsync(Forge* F)
    {
        ...
        // Add an asynchronous query callback
        F->GetQueryProcessor().AddCallback(CharacterDatabase.AsyncQuery(query).WithCallback([F, funcRef](QueryResult result)
        {
            ForgeQuery* eq = result ? &result : nullptr;
     ...
     * @return [ForgeQuery] results or nil if no rows found
     */
    int AuthDBQuery(Forge* F)
    {
        const char* query = F->CHECKVAL<const char*>(1);

        ForgeQuery result = LoginDatabase.Query(query);
        if (result)
            F->Push(&result);
        else
            F->Push();
    ...
    int AuthDBQueryAsync(Forge* F)
    {
        ...
        // Add an asynchronous query callback
        F->GetQueryProcessor().AddCallback(LoginDatabase.AsyncQuery(query).WithCallback([F, funcRef](QueryResult result)
        {
            ForgeQuery* eq = result ? &result : nullptr;
     ...
     */
    int CreateLuaEvent(Forge* F)
    {
        luaL_checktype(F->L, 1, LUA_TFUNCTION);
        uint32 min, max;
        if (lua_istable(F->L, 2))
        {
            F->Push(1);
            lua_gettable(F->L, 2);
            ...
            min = max = F->CHECKVAL<uint32>(2);
        uint32 repeats = F->CHECKVAL<uint32>(3, 1);

        if (min > max)
            return luaL_argerror(F->L, 2, "min is bigger than max delay");

        lua_pushvalue(F->L, 1);
        int functionRef = luaL_ref(F->L, LUA_REGISTRYINDEX);
        if (functionRef != LUA_REFNIL && functionRef != LUA_NOREF)
        {
            F->eventMgr->globalProcessor->AddEvent(functionRef, min, max, repeats);
            F->Push(functionRef);
        }
        return 1;
    }
     ...
     */
    int RemoveEventById(Forge* F)
    {
        int eventId = F->CHECKVAL<int>(1);
        bool all_Events = F->CHECKVAL<bool>(1, false);

        // not thread safe
        if (all_Events)
            F->eventMgr->SetState(eventId, LUAEVENT_STATE_ABORT);
        else
            F->eventMgr->globalProcessor->SetState(eventId, LUAEVENT_STATE_ABORT);
        return 0;
    }
     ...
     */
    int RemoveEvents(Forge* F)
    {
        bool all_Events = F->CHECKVAL<bool>(1, false);

        // not thread safe
        if (all_Events)
            F->eventMgr->SetStates(LUAEVENT_STATE_ABORT);
        else
            F->eventMgr->globalProcessor->SetStates(LUAEVENT_STATE_ABORT);
        return 0;
    }
     ...
     */
    int PerformIngameSpawn(Forge* F)
    {
        int spawntype = F->CHECKVAL<int>(1);
        uint32 entry = F->CHECKVAL<uint32>(2);
        uint32 mapID = F->CHECKVAL<uint32>(3);
        uint32 instanceID = F->CHECKVAL<uint32>(4);
        float x = F->CHECKVAL<float>(5);
        float y = F->CHECKVAL<float>(6);
        float z = F->CHECKVAL<float>(7);
        float o = F->CHECKVAL<float>(8);
        bool save = F->CHECKVAL<bool>(9, false);
        uint32 durorresptime = F->CHECKVAL<uint32>(10, 0);
        uint32 phase = F->CHECKVAL<uint32>(11, PHASEMASK_NORMAL);
     ...
     */
    int CreatePacket(Forge* F)
    {
        uint32 opcode = F->CHECKVAL<uint32>(1);
        size_t size = F->CHECKVAL<size_t>(2);
        if (opcode >= NUM_MSG_TYPES)
            return luaL_argerror(F->L, 1, "valid opcode expected");

        F->Push(new WorldPacket((OpcodesList)opcode, size));
        return 1;
    }
     ...
     */
    int AddVendorItem(Forge* F)
    {
        uint32 entry = F->CHECKVAL<uint32>(1);
        uint32 item = F->CHECKVAL<uint32>(2);
        int maxcount = F->CHECKVAL<int>(3);
        uint32 incrtime = F->CHECKVAL<uint32>(4);
        uint32 extendedcost = F->CHECKVAL<uint32>(5);
     ...
     * @param uint32 item : [Item] entry Id
     */
    int VendorRemoveItem(Forge* F)
    {
        uint32 entry = F->CHECKVAL<uint32>(1);
        uint32 item = F->CHECKVAL<uint32>(2);
        if (!eObjectMgr->GetCreatureTemplate(entry))
            return luaL_argerror(F->L, 1, "valid CreatureEntry expected");
     ...
     * @param uint32 entry : [Creature] entry Id
     */
    int VendorRemoveAllItems(Forge* F)
    {
        uint32 entry = F->CHECKVAL<uint32>(1);
    ...
    int Kick(Forge* F)
    {
        Player* player = F->CHECKOBJ<Player>(1);
    ...
    int Ban(Forge* F)
    {
        int banMode = F->CHECKVAL<int>(1);
        std::string nameOrIP = F->CHECKVAL<std::string>(2);
        uint32 duration = F->CHECKVAL<uint32>(3);
        const char* reason = F->CHECKVAL<const char*>(4, "");
        const char* whoBanned = F->CHECKVAL<const char*>(5, "");
        ...
        switch (banMode)
        {
            case BAN_ACCOUNT:
                if (!Utf8ToUpperOnlyLatin(nameOrIP))
                    return luaL_argerror(F->L, 2, "invalid account name");
                mode = BanMode::BAN_ACCOUNT;
                break;
            case BAN_CHARACTER:
                if (!normalizePlayerName(nameOrIP))
                    return luaL_argerror(F->L, 2, "invalid character name");
                mode = BanMode::BAN_CHARACTER;
                break;
            case BAN_IP:
                if (!IsIPAddress(nameOrIP.c_str()))
                    return luaL_argerror(F->L, 2, "invalid ip");
                mode = BanMode::BAN_IP;
                break;
            default:
                return luaL_argerror(F->L, 1, "unknown banmode");
        }
     ...
     */
    int SendMail(Forge* F)
    {
        int i = 0;
        std::string subject = F->CHECKVAL<std::string>(++i);
        std::string text = F->CHECKVAL<std::string>(++i);
        uint32 receiverGUIDLow = F->CHECKVAL<uint32>(++i);
        uint32 senderGUIDLow = F->CHECKVAL<uint32>(++i, 0);
        uint32 stationary = F->CHECKVAL<uint32>(++i, MAIL_STATIONERY_DEFAULT);
        uint32 delay = F->CHECKVAL<uint32>(++i, 0);
        uint32 money = F->CHECKVAL<uint32>(++i, 0);
        uint32 cod = F->CHECKVAL<uint32>(++i, 0);
        int argAmount = lua_gettop(F->L);
        ...
        {
            uint32 entry = F->CHECKVAL<uint32>(++i);
            uint32 amount = F->CHECKVAL<uint32>(++i);

            ItemTemplate const* item_proto = eObjectMgr->GetItemTemplate(entry);
            if (!item_proto)
            {
                luaL_error(F->L, "Item entry %d does not exist", entry);
                continue;
            }

            if (amount < 1 || (item_proto->MaxCount > 0 && amount > uint32(item_proto->MaxCount)))
            {
                luaL_error(F->L, "Item entry %d has invalid amount %d", entry, amount);
                continue;
            }
            if (Item* item = Item::CreateItem(entry, amount))
            {
                item->SaveToDB(trans);
                draft.AddItem(item);
                F->Push(item->GetGUID().GetCounter());
                ++addedItems;
            }
        }
    ...
    int bit_and(Forge* F)
    {
        uint32 a = F->CHECKVAL<uint32>(1);
        uint32 b = F->CHECKVAL<uint32>(2);
        F->Push(a & b);
        return 1;
    }
     ...
     */
    int AddTaxiPath(Forge* F)
    {
        luaL_checktype(F->L, 1, LUA_TTABLE);
        uint32 mountA = F->CHECKVAL<uint32>(2);
        uint32 mountH = F->CHECKVAL<uint32>(3);
        uint32 price = F->CHECKVAL<uint32>(4, 0);
        uint32 pathId = F->CHECKVAL<uint32>(5, 0);
        lua_pushvalue(F->L, 1);
        // Stack: {nodes}, mountA, mountH, price, pathid, {nodes}

        std::list<TaxiPathNodeEntry> nodes;

        int start = lua_gettop(F->L);
        int end = start;

        F->Push();
        // Stack: {nodes}, mountA, mountH, price, pathid, {nodes}, nil
        while (lua_next(F->L, -2) != 0)
        {
            ...
            if (end - start < 4) // no mandatory args, dont add
                return luaL_argerror(F->L, 1, "all waypoints do not have mandatory arguments");
            ...
            // mandatory
            entry.ContinentID = F->CHECKVAL<uint32>(start);
            entry.Loc.X = F->CHECKVAL<float>(start + 1);
            entry.Loc.Y = F->CHECKVAL<float>(start + 2);
            entry.Loc.Z = F->CHECKVAL<float>(start + 3);
            // optional
            entry.Flags = F->CHECKVAL<uint32>(start + 4, 0);
            entry.Delay = F->CHECKVAL<uint32>(start + 5, 0);

            nodes.push_back(entry);

            while (end != start) // remove args
                if (!lua_isnone(F->L, --end))
                    lua_remove(F->L, end);
            // Stack: {nodes}, mountA, mountH, price, pathid, {nodes}, key, value
     ...
     */
    int IsCompatibilityMode(Forge* F)
    {
        F->Push(F->GetCompatibilityMode());
        return 1;
    }
     ...
     */
    int IsInventoryPos(Forge* F)
    {
        uint8 bag = F->CHECKVAL<uint8>(1);
        uint8 slot = F->CHECKVAL<uint8>(2);

        F->Push(Player::IsInventoryPos(bag, slot));
        return 1;
    }
     ...
     */
    int IsEquipmentPos(Forge* F)
    {
        uint8 bag = F->CHECKVAL<uint8>(1);
        uint8 slot = F->CHECKVAL<uint8>(2);

        F->Push(Player::IsEquipmentPos(bag, slot));
        return 1;
    }
     ...
     */
    int IsBankPos(Forge* F)
    {
        uint8 bag = F->CHECKVAL<uint8>(1);
        uint8 slot = F->CHECKVAL<uint8>(2);

        F->Push(Player::IsBankPos(bag, slot));
        return 1;
    }
     ...
     */
    int IsBagPos(Forge* F)
    {
        uint8 bag = F->CHECKVAL<uint8>(1);
        uint8 slot = F->CHECKVAL<uint8>(2);

        F->Push(Player::IsBagPos((bag << 8) + slot));
        return 1;
    }
     ...
     */
    int IsGameEventActive(Forge* F)
    {
        uint16 eventId = F->CHECKVAL<uint16>(1);

        F->Push(eGameEventMgr->IsActiveEvent(eventId));
        return 1;
    }
     ...
     */
    int GetCurrTime(Forge* F)
    {
        F->Push(ForgeUtil::GetCurrTime());
        return 1;
    }
     ...
     */
    int GetTimeDiff(Forge* F)
    {
        uint32 oldtimems = F->CHECKVAL<uint32>(1);

        F->Push(ForgeUtil::GetTimeDiff(oldtimems));
        return 1;
    }

    static std::string GetStackAsString(Forge* F)
    {
        std::string output;
        int top = lua_gettop(F->L);
        for (int i = 1; i <= top; ++i)
        {
            if (lua_isstring(F->L, i))
            {
                output += lua_tostring(F->L, i);
            }
            else
            {
                lua_getglobal(F->L, "tostring");
                lua_pushvalue(F->L, i);
                lua_call(F->L, 1, 1);
                output += lua_tostring(F->L, -1);
                lua_pop(F->L, 1);
            }
     ...
     * @param ...
     */
    int PrintInfo(Forge* F)
    {
        FORGE_LOG_INFO("%s", GetStackAsString(F).c_str());
        return 0;
    }
     ...
     * @param ...
     */
    int PrintError(Forge* F)
    {
        FORGE_LOG_ERROR("%s", GetStackAsString(F).c_str());
        return 0;
    }
     ...
     * @param ...
     */
    int PrintDebug(Forge* F)
    {
        FORGE_LOG_DEBUG("%s", GetStackAsString(F).c_str());
        return 0;
    }
    ...
    */
    int StartGameEvent(Forge* F)
    {
        uint16 eventId = F->CHECKVAL<uint16>(1);
        bool force = F->CHECKVAL<bool>(2, false);
    ...
    */
     ...
     * @return int64 value
     */
    int CreateLongLong(Forge* F)
    {
        long long init = 0;
        if (lua_isstring(F->L, 1))
        {
            std::string str = F->CHECKVAL<std::string>(1);
            std::istringstream iss(str);
            iss >> init;
            if (iss.bad())
                return luaL_argerror(F->L, 1, "long long (as string) could not be converted");
        }
        else if (!lua_isnoneornil(F->L, 1))
            init = F->CHECKVAL<long long>(1);
    ...
    int CreateULongLong(Forge* F)
    {
        ...
        if (lua_isstring(F->L, 1))
        {
            ...
            iss >> init;
            if (iss.bad())
                return luaL_argerror(F->L, 1, "unsigned long long (as string) could not be converted");
        }
        else if (!lua_isnoneornil(F->L, 1))
            init = F->CHECKVAL<unsigned long long>(1);
     ...
     */
    int ClearBattleGroundEvents(Forge* F)
    {
        typedef EventKey<Hooks::BGEvents> Key;

        if (lua_isnoneornil(F->L, 1))
        {
            F->BGEventBindings->Clear();
        }
        else
        {
            uint32 event_type = F->CHECKVAL<uint32>(1);
            F->BGEventBindings->Clear(Key((Hooks::BGEvents)event_type));
        }
        return 0;
    }
     ...
     */
    int ClearCreatureEvents(Forge* F)
    {
        typedef EntryKey<Hooks::CreatureEvents> Key;

        if (lua_isnoneornil(F->L, 2))
        {
            uint32 entry = F->CHECKVAL<uint32>(1);

            for (uint32 i = 1; i < Hooks::CREATURE_EVENT_COUNT; ++i)
                F->CreatureEventBindings->Clear(Key((Hooks::CreatureEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->CreatureEventBindings->Clear(Key((Hooks::CreatureEvents)event_type, entry));
        }
        return 0;
    }
     ...
     * @param uint32 event_type : the event whose handlers will be cleared, see [Global:RegisterCreatureEvent]
     */
    int ClearUniqueCreatureEvents(Forge* F)
    {
        typedef UniqueObjectKey<Hooks::CreatureEvents> Key;

        if (lua_isnoneornil(F->L, 3))
        {
            ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);
            uint32 instanceId = F->CHECKVAL<uint32>(2);

            for (uint32 i = 1; i < Hooks::CREATURE_EVENT_COUNT; ++i)
                F->CreatureUniqueBindings->Clear(Key((Hooks::CreatureEvents)i, guid, instanceId));
        }
        else
        {
            ObjectGuid guid = F->CHECKVAL<ObjectGuid>(1);
            uint32 instanceId = F->CHECKVAL<uint32>(2);
            uint32 event_type = F->CHECKVAL<uint32>(3);
            F->CreatureUniqueBindings->Clear(Key((Hooks::CreatureEvents)event_type, guid, instanceId));
        }
        return 0;
    }
     ...
     */
    int ClearCreatureGossipEvents(Forge* F)
    {
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::GOSSIP_EVENT_COUNT; ++i)
                F->CreatureGossipBindings->Clear(Key((Hooks::GossipEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->CreatureGossipBindings->Clear(Key((Hooks::GossipEvents)event_type, entry));
        }
        return 0;
    }
     ...
     */
    int ClearGameObjectEvents(Forge* F)
    {
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::GAMEOBJECT_EVENT_COUNT; ++i)
                F->GameObjectEventBindings->Clear(Key((Hooks::GameObjectEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->GameObjectEventBindings->Clear(Key((Hooks::GameObjectEvents)event_type, entry));
        }
        return 0;
    }
     ...
     */
    int ClearGameObjectGossipEvents(Forge* F)
    {
        typedef EntryKey<Hooks::GossipEvents> Key;
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::GOSSIP_EVENT_COUNT; ++i)
                F->GameObjectGossipBindings->Clear(Key((Hooks::GossipEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->GameObjectGossipBindings->Clear(Key((Hooks::GossipEvents)event_type, entry));
        }
        return 0;
    }
     ...
     */
    int ClearGroupEvents(Forge* F)
    {
        typedef EventKey<Hooks::GroupEvents> Key;

        if (lua_isnoneornil(F->L, 1))
        {
            F->GroupEventBindings->Clear();
        }
        else
        {
            uint32 event_type = F->CHECKVAL<uint32>(1);
            F->GroupEventBindings->Clear(Key((Hooks::GroupEvents)event_type));
        }
        return 0;
    }
     ...
     */
    int ClearGuildEvents(Forge* F)
    {
        typedef EventKey<Hooks::GuildEvents> Key;

        if (lua_isnoneornil(F->L, 1))
        {
            F->GuildEventBindings->Clear();
        }
        else
        {
            uint32 event_type = F->CHECKVAL<uint32>(1);
            F->GuildEventBindings->Clear(Key((Hooks::GuildEvents)event_type));
        }
        return 0;
    }
    ...
    int ClearItemEvents(Forge* F)
    {
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::ITEM_EVENT_COUNT; ++i)
                F->ItemEventBindings->Clear(Key((Hooks::ItemEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->ItemEventBindings->Clear(Key((Hooks::ItemEvents)event_type, entry));
        }
        return 0;
    }
     ...
     */
    int ClearItemGossipEvents(Forge* F)
    {
        typedef EntryKey<Hooks::GossipEvents> Key;
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::GOSSIP_EVENT_COUNT; ++i)
                F->ItemGossipBindings->Clear(Key((Hooks::GossipEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->ItemGossipBindings->Clear(Key((Hooks::GossipEvents)event_type, entry));
        }
        return 0;
    }
    ...
    int ClearPacketEvents(Forge* F)
    {
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::PACKET_EVENT_COUNT; ++i)
                F->PacketEventBindings->Clear(Key((Hooks::PacketEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->PacketEventBindings->Clear(Key((Hooks::PacketEvents)event_type, entry));
        }
        return 0;
    }
     ...
     */
    int ClearPlayerEvents(Forge* F)
    {
        typedef EventKey<Hooks::PlayerEvents> Key;

        if (lua_isnoneornil(F->L, 1))
        {
            F->PlayerEventBindings->Clear();
        }
        else
        {
            uint32 event_type = F->CHECKVAL<uint32>(1);
            F->PlayerEventBindings->Clear(Key((Hooks::PlayerEvents)event_type));
        }
        return 0;
    }
     ...
     */
    int ClearPlayerGossipEvents(Forge* F)
    {
        typedef EntryKey<Hooks::GossipEvents> Key;
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::GOSSIP_EVENT_COUNT; ++i)
                F->PlayerGossipBindings->Clear(Key((Hooks::GossipEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->PlayerGossipBindings->Clear(Key((Hooks::GossipEvents)event_type, entry));
        }
        return 0;
    }
     ...
     */
    int ClearServerEvents(Forge* F)
    {
        typedef EventKey<Hooks::ServerEvents> Key;

        if (lua_isnoneornil(F->L, 1))
        {
            F->ServerEventBindings->Clear();
        }
        else
        {
            uint32 event_type = F->CHECKVAL<uint32>(1);
            F->ServerEventBindings->Clear(Key((Hooks::ServerEvents)event_type));
        }
        return 0;
    }
    ...
    int ClearMapEvents(Forge* F)
    {
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::INSTANCE_EVENT_COUNT; ++i)
                F->MapEventBindings->Clear(Key((Hooks::InstanceEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->MapEventBindings->Clear(Key((Hooks::InstanceEvents)event_type, entry));
        }
     ...
     */
    int ClearInstanceEvents(Forge* F)
    {
        typedef EntryKey<Hooks::InstanceEvents> Key;
        ...
        if (lua_isnoneornil(F->L, 2))
        {
            ...
            for (uint32 i = 1; i < Hooks::INSTANCE_EVENT_COUNT; ++i)
                F->InstanceEventBindings->Clear(Key((Hooks::InstanceEvents)i, entry));
        }
        else
        {
            uint32 entry = F->CHECKVAL<uint32>(1);
            uint32 event_type = F->CHECKVAL<uint32>(2);
            F->InstanceEventBindings->Clear(Key((Hooks::InstanceEvents)event_type, entry));
        }

        return 0;
    }

    ForgeRegister<> GlobalMethods[] =
    {
        // Hooks
