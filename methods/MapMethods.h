...
namespace LuaMap
{
     ...
     * @return bool isArena
     */
    int IsArena(Forge* F, Map* map)
    {
        F->Push(map->IsBattleArena());
        return 1;
    }
     ...
     */
    int IsBattleground(Forge* F, Map* map)
    {
        F->Push(map->IsBattleground());
        return 1;
    }
     ...
     */
    int IsDungeon(Forge* F, Map* map)
    {
        F->Push(map->IsDungeon());
        return 1;
    }
     ...
     */
    int GetName(Forge* F, Map* map)
    {
        F->Push(map->GetMapName());
        return 1;
    }
     ...
     * @return float z
     */
    int GetHeight(Forge* F, Map* map)
    {
        float x = F->CHECKVAL<float>(2);
        float y = F->CHECKVAL<float>(3);
        uint32 phasemask = F->CHECKVAL<uint32>(4, 1);
     ...
     */
    int GetDifficulty(Forge* F, Map* map)
    {
        F->Push(map->GetDifficulty());
        return 1;
    }
     ...
     */
    int GetInstanceId(Forge* F, Map* map)
    {
        F->Push(map->GetInstanceId());
        return 1;
    }
     ...
     */
    int GetPlayerCount(Forge* F, Map* map)
    {
        F->Push(map->GetPlayersCountExceptGMs());
        return 1;
    }
     ...
     */
    int GetAreaId(Forge* F, Map* map)
    {
        float x = F->CHECKVAL<float>(2);
        float y = F->CHECKVAL<float>(3);
        float z = F->CHECKVAL<float>(4);
        float phasemask = F->CHECKVAL<uint32>(5, PHASEMASK_NORMAL);

        F->Push(map->GetAreaId(phasemask, x, y, z));
        return 1;
    }
     ...
     */
    int GetWorldObject(Forge* F, Map* map)
    {
        ...
        switch (guid.GetHigh())
        {
            case HIGHGUID_PLAYER:
                F->Push(eObjectAccessor()GetPlayer(map, guid));
                break;
            case HIGHGUID_TRANSPORT:
            case HIGHGUID_MO_TRANSPORT:
            case HIGHGUID_GAMEOBJECT:
                F->Push(map->GetGameObject(guid));
                break;
            case HIGHGUID_VEHICLE:
            case HIGHGUID_UNIT:
                F->Push(map->GetCreature(guid));
                break;
            case HIGHGUID_PET:
                F->Push(map->GetPet(guid));
                break;
            case HIGHGUID_DYNAMICOBJECT:
                F->Push(map->GetDynamicObject(guid));
                break;
            case HIGHGUID_CORPSE:
                F->Push(map->GetCorpse(guid));
                break;
            default:
                break;
        }
     ...
     */
    int SetWeather(Forge* F, Map* map)
    {
        (void)map; // ensure that the variable is referenced in order to pass compiler checks
        uint32 zoneId = F->CHECKVAL<uint32>(2);
        uint32 weatherType = F->CHECKVAL<uint32>(3);
        float grade = F->CHECKVAL<float>(4);
     ...
     */
    int GetInstanceData(Forge* F, Map* map)
    {
     ...
     */
    int SaveInstanceData(Forge* /*E*/, Map* map)
    {
        ForgeInstanceAI* iAI = NULL;
        if (InstanceMap* inst = map->ToInstanceMap())
            iAI = dynamic_cast<ForgeInstanceAI*>(inst->GetInstanceScript());
    ...
    */
    int GetPlayers(Forge* F, Map* map)
    {
        uint32 team = F->CHECKVAL<uint32>(2, TEAM_NEUTRAL);
     ...
     * </pre>
     */
    int Data(Forge* F, Map* map)
    {
        return LuaVal::PushLuaVal(F->L, map->lua_data);
    }
